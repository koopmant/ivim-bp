function [fr,gof,fDt,xiDt] = fit_IVIMs0_BP(bvec,S,~,varargin)
...
%priors
priorDp = @(Dp) (Dp > 0 & Dp < 1);
priorDt = @(Dt) (Dt > 0 & Dt < 1);
priorFp = @(fp) (fp > 0 & fp < 1);
priorS0 = @(s0) (s0 > 0);
...
nKernelPoints = 1000;
if ~isempty(varargin)
    initialP = [varargin{1}(1),varargin{1}(2),varargin{1}(3),varargin{1}(4)];
else
    initialP = magP;
end
